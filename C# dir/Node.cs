...
namespace CE301___Attempt_2
{
    class Node
    {
        ...
        public Node changeThreshold(Node[] nodes, out int index)             //Mutator
        {
            ...
            if (nodes[i].children != null)                                                  //Ensure we do not select a leaf node
            {                                                                                    
        ...
        public Node changeAttribute(Node[] nodes, out int index)                                //Mutator
        {
            ...
            if (selected.label == 0)               //Ensure we do not select a leaf node 
            {
        ...
        public Node randomMutateAndRebuild(Node[] nodes)    //Takes a flattened tree and randomly breaks and rebuilds a branch
        {
            ...
            if (mutatorID == 0)
            {                                 //Manually sort subTable through the mutated value
            ...
            return nodes[0];                                                                        //Returns the root of the altered tree 
