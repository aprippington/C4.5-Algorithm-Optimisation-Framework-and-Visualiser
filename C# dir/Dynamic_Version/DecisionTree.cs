...
namespace Decision_Tree_Optimisation_Generalised
{
    class DecisionTree
    {
        ...
        public Node RunC4_5(DataTable x, List<DataColumn> attributes)    //C4.5 ALGORITHM
        {           
            Node n = new Node();
            count += 1; 
            n.subTable = x;
            ...
            ///////////////////SHOULD NEVER ENTER THIS LOOP UNLESS PARTITION RETURNED FULL DATASET ONE SIDE AND EMPTY THE OTHER///////////////////////////////////           
            /*
            if (AllValuesTheSame(x, bestAttribute.Ordinal))
            {                
                attributes.Remove(bestAttribute);
                //n = RunC4_5(x, attributes);
                n.label = GetMostCommonLabel(x);
                return n;
            }
            */

            //Console.WriteLine("bestAttribute this iteration is:   " + bestAttribute.ColumnName + "   " + bestAttribute.DataType);   //FOR TESTING
        ...
        static void Main(string[] args)
        {
            ...
            root.isRoot = true;  //Identify the root 
            Accuracy a = new Accuracy();
            a.GetAccuracy(root);

            Node[] nodes = root.flattenTree(root);
            Console.WriteLine("Total nodes in this tree:  " + nodes.Length);
            //foreach(Node n in nodes) { Console.WriteLine("Node attribute:  " + n.category + "   Node label: " + n.label); }
            ...
            //Try xxx iterationss of optimisation cycle          
           double maxAcc = a.GetAccuracy(root);
            ...
            {                                
                ...
                root = root.randomMutateAndRebuild(root);
                if (a.GetAccuracy(root) > maxAcc) { maxAcc = a.GetAccuracy(root); }
            }
            Console.WriteLine("Max Accuracy found = " + maxAcc);
            /*
            double aaa = a.GetAccuracy(root);
            for (int i = 0; i < 100; i++)
            {
                Console.WriteLine("FINAL ACCURACY = " + a.GetAccuracy(root));
                if (a.GetAccuracy(root) != aaa)
                {
                    throw new Exception();
                }
            }*/
            ...
            /*////Print Column DataTypes and full DataSet /////////////
            foreach (DataColumn c in x.Columns) { Console.WriteLine("Column no.: " + c.Ordinal + "   Column name: " + c.ColumnName + "   Data Type: " + c.DataType); }
            Console.WriteLine("Dataset(x):");
            foreach (DataRow r in x.Rows)
            {
                int i = 0;
                Console.WriteLine(r[i].ToString() + "  " + r[i + 1].ToString() + "  " + r[i + 2].ToString() + "  " + r[i + 3].ToString() + "  " + r[i + 4].ToString() + "  " +
                    r[i + 5].ToString() + "  " + r[i + 6].ToString() + "  " + r[i + 7].ToString() + "  " + r[i + 8].ToString() + "  " + r[i + 9].ToString() + "  " + r[i + 10].ToString()
                    + "  " + r[i + 11].ToString() + "  " + r[i + 12].ToString() + "  " + r[i + 13].ToString());
            }
            ///////////////////////////////////////////////////////////
            //*/
            ...
            /* ///////////////////////////////////
            a.GetAccuracy(root);
            root.setDepth(root, 0);

            Node[] nodes = root.flattenTree(root);                     
            root.visualiseTree(nodes);
            //////////
            /*/////////////////////////////////////
